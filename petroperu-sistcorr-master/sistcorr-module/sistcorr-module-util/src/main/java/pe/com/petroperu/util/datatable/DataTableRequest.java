...
public class DataTableRequest<T> {
	...
	private void prepareDataTableRequest(HttpServletRequest request) {
    	...
    	if(parameterNames.hasMoreElements()) {
    		...
    		if(!ApplicationUtil.isObjectEmpty(this.getSearch())) {
    		...
    		for(int i=0; i < maxParamsToCheck; i++) {
    			if(null != request.getParameter("columns["+ i +"][data]") 
    					...
    					&& !ApplicationUtil.isObjectEmpty(request.getParameter("columns["+ i +"][data]"))) {
    				...
    				if(!ApplicationUtil.isObjectEmpty(colSpec.getSearch())) {
    					this.setGlobalSearch(false);
    				}
    			} 
    		}
    		
    		if(!ApplicationUtil.isObjectEmpty(columns)) {
	...
	public PaginationCriteria getPaginationRequest() {
		...
		SortBy sortBy = null;
		if(!ApplicationUtil.isObjectEmpty(this.getOrder())) {
			sortBy = new SortBy();
		...
		for(DataTableColumnSpecs colSpec : this.getColumns()) {
			if(colSpec.isSearchable()) {
				if(!ApplicationUtil.isObjectEmpty(this.getSearch()) || !ApplicationUtil.isObjectEmpty(colSpec.getSearch())) {
