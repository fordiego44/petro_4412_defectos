...
public class DataTableRequestCGC extends DataTableRequest<Ciudad>{

	private CgcCorrespondencia filtro;
	private String columna;
	private String ordenDesc;
	public CgcCorrespondencia getFiltro() {
		return filtro;
	}
	public void setFiltro(CgcCorrespondencia filtro) {
		this.filtro = filtro;
	}
	...
	public DataTableRequestCGC(HttpServletRequest request) {
		super(request);
	...
	private void prepareRequestFilter(HttpServletRequest request) {
		this.filtro = new CgcCorrespondencia();
		
		try {
			this.filtro.setNombreCGC((String) request.getParameter("nombreCGC"));
		} catch(Exception e){
			this.filtro.setNombreCGC("");
		}
		...
		try{
			...
			String columnaOrden = Constante.COLUMNAS_CONSULTA_CGC[columnOrder - 2];
