...
public class DataTableRequestLogConsulta extends DataTableRequest<FiltroConsultaLogDTO> {
	
	private FiltroConsultaLogDTO filtro;
	...
	public FiltroConsultaLogDTO getFiltro() {
		return filtro;
	}
	
	public void setFiltro(FiltroConsultaLogDTO filtro) {
		this.filtro = filtro;
	}
	
	public DataTableRequestLogConsulta(HttpServletRequest request){
		super(request);
		prepareRequestFilter(request);
	}
	
	private void prepareRequestFilter(HttpServletRequest request){
		this.filtro = new FiltroConsultaLogDTO();
		try {
			this.filtro.setTabla((String) request.getParameter("tabla"));
		} catch(Exception e) {
			this.filtro.setTabla("");
		}
		try {
			this.filtro.setUsuario((String) request.getParameter("usuario"));
		} catch(Exception e) {
			this.filtro.setUsuario("");
		}
		try {
			this.filtro.setAccion((String) request.getParameter("accion"));
		} catch(Exception e) {
			this.filtro.setAccion("");
		}
