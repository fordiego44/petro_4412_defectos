...
public class DataTableRequestPais extends DataTableRequest<Pais> {
	...
	public DataTableRequestPais(HttpServletRequest request) {
		super(request);
	...
	private void prepareRequestFilter(HttpServletRequest request) {
		this.filtro = new Pais();
		
		try {
			this.filtro.setNombrePais((String) request.getParameter("nombrePais"));
		} catch(Exception e){
			this.filtro.setNombrePais("");
		}
		...
		try{
			...
			String columnaOrden = Constante.COLUMNAS_CONSULTA_PAISES[columnOrder - 2];
