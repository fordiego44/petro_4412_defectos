...
public class DataTableRequestTipoComprobante extends DataTableRequest<TipoComprobante>{

	private TipoComprobante filtro;
	private String columna;
	private String ordenDesc;
	
	public TipoComprobante getFiltro() {
		return filtro;
	}
	public void setFiltro(TipoComprobante filtro) {
		this.filtro = filtro;
	}
	...
	public DataTableRequestTipoComprobante(HttpServletRequest request) {
		super(request);
	...
	private void prepareRequestFilter(HttpServletRequest request) {
		this.filtro = new TipoComprobante();
		
		try {
			this.filtro.setNombreComprobante((String) request.getParameter("nombreComprobante"));
		} catch(Exception e){
			this.filtro.setNombreComprobante("");
		}
		...
		try{
			...
			String columnaOrden = Constante.COLUMNAS_CONSULTA_TIPO_COMPROBANTE[columnOrder - 2];
