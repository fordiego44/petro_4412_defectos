...
public class DataTableRequestMotivo extends DataTableRequest<Motivo>  {

	private Motivo filtro;
	private String columna;
	private String ordenDesc;
	
	public Motivo getFiltro() {
		return filtro;
	}

	public void setFiltro(Motivo filtro) {
		this.filtro = filtro;
	}
	...
	public DataTableRequestMotivo(HttpServletRequest request) {
		super(request);
	...
	private void prepareRequestFilter(HttpServletRequest request) {
		this.filtro = new Motivo();
		
		try {
			this.filtro.setNombreMotivo((String) request.getParameter("nombreMotivo"));
		} catch(Exception e){
			this.filtro.setNombreMotivo("");
		}
		...
		try{
			...
			String columnaOrden = Constante.COLUMNAS_CONSULTA_MOTIVOS[columnOrder - 2];
