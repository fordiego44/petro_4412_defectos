var MODULO_RESPUESTA_FIRMA = (function(){
	...
	function ModuloRespuestaFirma(SISCORR_APP){
		...
		this.URL_PROCESAR_EXISTENCIA_ENLACE = '../../app/emision/procesar-existencia-enlace';
	}
	...
	ModuloRespuestaFirma.prototype.enviarCorrespondencia = function(){
		var ref = this;
		var token = ref.csrfToken.val(); 
		var header = ref.csrfHeader.val();
		var idCorrespondenica = ref.idCorrespondencia.val();
		return $.ajax({
			type	:	'PUT',
	...
	ModuloRespuestaFirma.prototype.procesarExistenciaEnlace = function(){
		var ref = this;
		var token = ref.csrfToken.val(); 
		var header = ref.csrfHeader.val();
		var idCorrespondenica = ref.idCorrespondencia.val();
		return $.ajax({
			type	:	'PUT',
			url		: 	ref.URL_PROCESAR_EXISTENCIA_ENLACE + '/' + idCorrespondenica,
			cache	:	false,
			beforeSend: function(xhr) {
		        xhr.setRequestHeader("Accept", "application/json");
		        xhr.setRequestHeader("Content-Type", "application/json");
