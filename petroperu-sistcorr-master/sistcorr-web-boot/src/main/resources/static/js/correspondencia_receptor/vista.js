...
var CORRESPONDENCIA_VISTA = {
		...
		iniciarEventos: function(){
			...
			ref.modulo.compBtnAsignarDestinatario.click(function(event){
		...
		asignarDestinatario: function(){
			...
			var dependenciaDestino=ref.modulo.compDependenciaDestino.val();
			
			var accion="";
			if (ref.correspondenciaPorAsignar && ref.correspondenciaPorAsignar.dependenciadestino==""){
				accion="A";
			}else{
				accion="R";
			}
			...
			ref.modSistcorr.procesar(true);
			ref.modulo.asignarDestinatario(ref.correspondenciaPorAsignar.correlativo, cgc,dependencia,accion)
				.then(function(respuesta){
		...
		abrirModalBusqueda: function(){
			...
			if(ref.filtrosBusqueda.length > 0){
				for(var i in ref.filtrosBusqueda){
					...
					} else if(field.fieldId == 'TD_DependenciaDest' && ref.modulo.compTipoBandeja.val()=="DelGestor"){
		...
		obtenerFiltrosBusqueda: function(){
			...
			if(ref.modulo.compTipoBandeja.val()=="DelGestor"){
				if(dependenciaDestino.length > 0){
					data.push({'fieldId' : 'TD_DependenciaDest', "operator": "%LIKE%", "value": (dependenciaDestino || "").toUpperCase()});
				}
			}
			// FIN TICKET 
			if(asunto.length > 0){
		...
		eventoSeleccionarCorrespondencia: function(){
			...
			$("."+ref.modulo.compAbrirAsignarCorrespondencia).click(function(){
				var $comp = $(this);
				var correlativo = $comp.data("correlativo");
				var dependenciaDest = $comp.data("dependenciadestino");
				ref.correspondenciaPorAsignar = {
						'correlativo' : 	correlativo,
						'dependenciadestino': dependenciaDest
				};
				ref.abrirModalAsignarCorrespondencia();
			});
		...
		actualizarCombosDependenciaDestino: function(){
			...
			for(var index in dependencias){
				var dependencia = dependencias[index];
				if(selected){
					val = dependencia.descripcion;
					ref.modulo.compBusqueda.dependenciaDestino.append("<option value='"+dependencia.descripcion+"' selected='selected'>" + dependencia.descripcion + "</option>");
				}else{
					ref.modulo.compBusqueda.dependenciaDestino.append("<option value='"+dependencia.descripcion+"'>" + dependencia.descripcion + "</option>");
				}
			}
			if(selected){
		...
		abrirModalAsignarCorrespondencia: function(){
			var ref = this;
			ref.modSistcorr.procesar(true);
			ref.modulo.compDependencia.val("");
			ref.modulo.compDependencia.change();
			ref.modulo.compLugarTrabajo.val("");
			ref.modulo.compLugarTrabajo.change();
			//ref.modulo.compComentarioRechazarCorrespondencia.val("");
			ref.modulo.compModalAsignarCorrespondencia.modal('show');
			setTimeout(function() {
				//ref.modulo.compComentarioRechazarCorrespondencia.focus();
		...
		rechazarCorrespondencia: function(){
			...
			ref.modSistcorr.procesar(true);
			ref.modulo.rechazarCorrespondenciaMPV(ref.correspondenciaPorRechazar.correlativo, comentario)
				.then(function(respuesta){
					if(respuesta.estado == true){
						ref.modSistcorr.procesar(false);
						ref.modSistcorr.notificar("OK", respuesta.mensaje, "Success");
						// TICKET 9000003997
						/*if(ref.esDeEmisionSeleccionada=="SI"){
							ref.modulo.procesarRechazarCorrespondenciaMPV(ref.numeroDocumentoSeleccionado, ref.correlativoSeleccionado, comentario)
								.then(function(respuesta){
									console.log(respuesta)
								}).catch(function(error){
									ref.modSistcorr.procesar(false);
		...
		iniciarCombosAutoCompletados: function(){
			...
			/*var rmtLugarTrabajoSelect2 = ref.modulo.compLugarTrabajo.select2({
				ajax: {
				    url: ref.modulo.URL_BUSCAR_LUGARES,
				    data: function (params) {
				        var query = {
			...
			var rmtDependenciaSelect2 = ref.modulo.compDependencia.select2({
				ajax: {
			...
			var destInternoTrabajoSelect2 = ref.modulo.compLugarTrabajo.select2({
				ajax: {
				    url: ref.modulo.URL_BUSCAR_LUGARES,
				    data: function (params) {
				        var query = {
			...
			var desInternoDependenciaSelect2 = ref.modulo.compDependencia.select2({
				ajax: {
				    url: ref.modulo.URL_BUSCAR_DEPENDENCIAS,
				    data: function (params) {
				        var query = {
...
setTimeout(function(){
	...
	CORRESPONDENCIA_VISTA.listas = {};
	CORRESPONDENCIA_VISTA.listas.lugarTrabajo =  new LISTA_DATA([]);
	CORRESPONDENCIA_VISTA.listas.dependencia =  new LISTA_DATA([]);
}, 500);