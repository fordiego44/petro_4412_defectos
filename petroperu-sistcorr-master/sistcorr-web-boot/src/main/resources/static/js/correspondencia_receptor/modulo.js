var MODULO_CORRESPONDENCIA = (function(){
	...
	function ModuloCorrespondencia(SISCORR_APP){
		...
		this.compUrl = $("#url");
		this.compDependenciaDestino=$("#dependenciaDestino");
		...
		this.compAbrirAsignarCorrespondencia="btnAsignarCorrespondencia";
		this.compModalRechazarCorrespondencia = $("#modalRechazarCorrespondencia");
		this.compModalAsignarCorrespondencia = $("#modalAsignarCorrespondencia");
		this.compBtnRechazarCorrespondencia = $("#btnRechazarCorres");
		this.compBtnAsignarCorrespondencia=$("#btnAsignarCorrespondencia");
		this.compComentarioRechazarCorrespondencia = $("#textRechazarCorrespondencia");
		this.plantillaCorrespondencias = $("#template-correspondencias-receptor");
		...
		this.compBtnAsignarDestinatario = $("#btnGuardarAsignacion");
	...
	ModuloCorrespondencia.prototype.asignarDestinatario = function(correlativo, cgc,dependencia,accion){
		var ref = this;
		...
		if(correlativoDef==""){
			return $.ajax({
				type		:	'PUT',
				url			:	'../' + ref.URL_BASE_CORRESPONDENCIA + '/asignaciones/asignarDependenciaMPV/' + correlativo,
	...
	ModuloCorrespondencia.prototype.rechazarCorrespondenciaMPV = function(correlativo, observacion){
		var ref = this;
		var token = ref.csrfToken.val(); 
		var header = ref.csrfHeader.val();
	...
	ModuloCorrespondencia.prototype.procesarRechazarCorrespondenciaMPV = function(nroDocumento, correlativo, comentario){
		var ref = this;
		var token = ref.csrfToken.val(); 
		var header = ref.csrfHeader.val();
	...
	ModuloCorrespondencia.prototype.htmlCorrespondencias = function(correspondencias, rol_jefe, rol_gestor){
		...
		Handlebars.registerHelper('estiloNO_SA', function(tipoIcono){
			if(ref.compTipoBandeja.val() == "DelReceptor"){
		...
		Handlebars.registerHelper('estilo_opciones_SA_PA', function(corres){
            ...
            if(nuevaCorrespondencia == corres.tipoIcono || porAceptar == corres.tipoIcono){
                return "display: block;";
            }
            return "display: none; cursor: default;";
			}
		...
		Handlebars.registerHelper('estilo_opciones_SA_PA_descarga', function(corres){
            ...
            if((nuevaCorrespondencia == corres.tipoIcono || porAceptar == corres.tipoIcono) && (rol_jefe == true || (rol_gestor == true && (!(corres && corres.esConfidencial == "SI" && (corres.esAsignado == null || corres.esAsignado == "NO" || corres.esAsignado == "")))) || (rol_jefe == false && rol_gestor == false && (corres.esAsignado == "SI" || corres.esConfidencial == "NO")))){
                return "display: block;";
            }
            return "display: none; cursor: default;";
			}
		...
		Handlebars.registerHelper('estilo_opciones_SA_PA_copia', function(corres){
			...
			if((nuevaCorrespondencia == corres.tipoIcono || porAceptar == corres.tipoIcono) && (corres && corres.esConfidencial == "NO")){
                return "display: block;";
            }
            return "display: none; cursor: default;";
			}
		...
		Handlebars.registerHelper('estilo_opciones_NoSAR', function(corres){
            ...
            if(nuevaCorrespondencia != corres.tipoIcono && corres.sVersionNum >= VERSION_BAND_ENTRADA_ASIG){
                return "display: block;";
            }
            return "display: none; cursor: default;";
			}
		...
		Handlebars.registerHelper('estilo_Rechazado', function(corres){
			
			/*if(corres.dependenciaDestino == ""){
				return "display: none; cursor: default;"
			}else{
				return "display: block;";
			}*/
		...
		Handlebars.registerHelper('estilo_NoPA_RHZASIG', function(corres){
			...
			if((ref.compTipoBandeja.val() == "Pendientes" || ref.compTipoBandeja.val() == "EnAtencion") &&
					(nuevaCorrespondencia != corres.tipoIcono && corres.sVersionNum >= VERSION_BAND_ENTRADA_ASIG) &&
					(corres.esAsignado && corres.esAsignado == "SI")){
				return "display: block;"
			}
            return "display: none; cursor: default;";
        });
		...
		Handlebars.registerHelper('url_pagina_detalle_correspondencia_receptor', function(correspondencia){
			var correlativo = $("#correlativoDef").val();
			if(correlativo==""){
	...
	ModuloCorrespondencia.prototype.htmlFiltros = function(filtros){
		...
		for(var i in items){
			var f = items[i];
			// TICKET 9000003862
			if(f.fieldId == 'TD_DependenciaDest'){
				f.text = 'Dependencia Destino: ' + f.value;
			}
			// FIN TICKET
