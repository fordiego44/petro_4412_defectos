var modulo_correspondencia_asignacion_grupal = MODULO_CORRESPONDENCIA_ASIGNACION_GRUPAL.getInstance(SISTCORR);
var modulo_sistcorr = MODULO_SISTCORR.getInstance(SISTCORR);

var CORRESPONDENCIA_ASIGNACION_GRUPAL = {
	modulo: null,
	modSistcorr: null,
	...
	validarDatosAsignacionDesdeEnviar: function(){
		...
		if(funcionarioResponsable == '') return false;
		if(detalleReq == '' && requiereTexto == true) return false;
	...
	enviarAsignaciones: function(){
		...
		if(ref.validarDatosAsignacionDesdeEnviar() && ref.asignaciones.length == 0){
			...
			if(ref.modulo.VISTA.compAsignaciones.children().length != 0) {
				    //alert('Yes content');
				ref.modulo.enviarAsignaciones()
				.then(function(respuesta){
					if(respuesta.estado == true){
		    	...
		    	ref.modSistcorr.procesar(false);
				ref.modSistcorr.notificar("ERROR" , "DEBE INGRESAR AL MENOS UNA ASIGNACIÓN!" ,"Error");
			}
		}
	},
	...
	regresaraBandeja: function(){
		var ref = this;
		var ruta = ref.modulo.tipoBandeja.val();
		window.location.href = window.location.protocol + "//" + window.location.hostname + (window.location.port ? ':' + window.location.port: '') + "/"+window.location.pathname.split("/")[1]+"/app/" + ((ruta== "Pendientes")?("pendientes"):((ruta== "EnAtencion")?("atencion"):((ruta== "Completadas")?("completadas"):(""))));	
	},
...
setTimeout(function() {
	CORRESPONDENCIA_ASIGNACION_GRUPAL.modulo = modulo_correspondencia_asignacion_grupal;
	CORRESPONDENCIA_ASIGNACION_GRUPAL.modSistcorr = modulo_sistcorr;
	CORRESPONDENCIA_ASIGNACION_GRUPAL.correspondencia = CORRESPONDENCIA;
	CORRESPONDENCIA_ASIGNACION_GRUPAL.errores = ERRORES || [];
	CORRESPONDENCIA_ASIGNACION_GRUPAL.utilitario_textMas = utilitario_textLarge;
	CORRESPONDENCIA_ASIGNACION_GRUPAL.inicializar();
}, 200);