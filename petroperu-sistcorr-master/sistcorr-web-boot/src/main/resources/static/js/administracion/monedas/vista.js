var modulo_mantenimiento_monedas = MODULO_MANTENIMIENTO_MOMEDAS.getInstance(SISTCORR);
var modulo_sistcorr = MODULO_SISTCORR.getInstance(SISTCORR);

var MANTENIMIENTO_MONEDAS_VISTA = {
		modSistcorr: null,
		modulo: null,
		errores: [],
		rol_jefe: false,
		...
		inicializar: function(){
			...
			var filtros = JSON.parse(sessionStorage.getItem('FILTROS_MONEDAS'));
			if(filtros!=null){
		...
		iniciarEventos: function(){
			...
			ref.modulo.btnRegistrar.click(function(event){
				...
				ref.modulo.compModalDatos.codigoMoneda.val("");
				ref.modulo.compModalDatos.codigoMoneda.change();
		...
		eventoDocumentoPrincipal: function(){
			...
			ref.modulo.btnGrillaEditar.off('click').on('click', function(){
				...
				var comp = $(this);
				var idMoneda = comp.attr("data-monedaid");
				
				for(var i in datosConsulta){
					var datoMoneda = datosConsulta[i];
					if(datoMoneda.id == idMoneda){
						ref.modulo.compModalDatos.id = datoMoneda.id;
						ref.modulo.compModalDatos.accion = "U";
						ref.modulo.compModalDatos.codigoMoneda.val(datoMoneda.codigoMoneda);
						ref.modulo.compModalDatos.codigoMoneda.change();
						ref.modulo.compModalDatos.descripcion.val(datoMoneda.descripcion);
						ref.modulo.compModalDatos.descripcion.change();
						break;
					}
				}
			...
			ref.modulo.btnGrillaEliminar.off('click').on('click', function(){
				var comp = $(this);
				var idMoneda = comp.attr("data-monedaid");
				ref.modulo.compModalDatos.id = idMoneda;
				setTimeout(function(){
					ref.modulo.compModalEliminar.modal('show');
				}, 300);
			}),
		...
		buscarMoneda: function(){
			...
			/*if(!valido){
				...
				sessionStorage.setItem("FILTROS_MONEDAS", JSON.stringify(filtrosBusqueda));
				ref.inicializarTablaConsulta();
			//}	
		},
		
		
		eliminarMoneda: function(){
			var ref = this;
			ref.parametros = {
		...
		insertUpdateMonedas: function(){
			var ref = this;
			ref.modSistcorr.procesar(true);
			if(ref.modulo.compModalDatos.codigoMoneda.val() == ""){
				//valido = true;
			...
			if(ref.modulo.compModalDatos.descripcion.val() == ""){
			...
			ref.parametros = {
					'id' : 	ref.modulo.compModalDatos.id,
					'codigoMoneda' : ref.modulo.compModalDatos.codigoMoneda.val(),
			...
			ref.modulo.exportarExcel(ref.filtrosBusqueda)
				.then(function(respuesta){
					if (navigator.appVersion.toString().indexOf('.NET') > 0){
						window.navigator.msSaveOrOpenBlob(respuesta, 'reporte_monedas.xlsx');
					} else {
		...
		inicializarTablaConsulta: function(){
			...
			if(ref.dataTableConsulta){
				...
				ref.dataTableConsulta = ref.modulo.compBusqueda.dataTableConsulta.DataTable({
			        ...
			        "ordering"	: true,
			        ajax: {
			        	"url": ref.modulo.URL_BUSCAR_MONEDA,
			        	"type": "GET",
			        	"data": {
			        		'descripcion' : ref.modulo.filtroBusqueda.filtroDescripcion.val(),
			        	...
			        	'error': function(result){
			        		console.log("Error Consulta Monedas");
			        		console.log(result);
			        		ref.modSistcorr.procesar(false);
			        		//location.reload();
			        	}
			        ...
			        "columns": [
			        	{data: 'id', title: '', defaultContent: '', render: function(data, type, full){
			        		return "<i class='fas fa-edit icon_editar btnGrillaEditar' data-monedaid='"+full.id+"' data-toggle='tooltip' title='Editar'></i>"

			        	}},
			        	{data: 'id', title: '', defaultContent: '', render: function(data, type, full){
			        		return "<i class='fas fa-trash-alt icon_eliminar btnGrillaEliminar' data-monedaid='"+full.id+"' data-toggle='tooltip' title='Eliminar'></i>"
			        	}},
			        	{data: 'id', title: 'Id', defaultContent: ''},
			        	{data: 'codigoMoneda', title: 'Código Moneda', defaultContent: ''},
			        	{data: 'descripcion', title: 'Descripción Moneda', defaultContent: ''}
				...
				ref.modulo.compBusqueda.dataTableConsulta.on( 'order.dt', function () {
					var tableOrder = $("#tablaMonedas").dataTable();
...
setTimeout(function(){
	MANTENIMIENTO_MONEDAS_VISTA.modSistcorr = modulo_sistcorr;
	MANTENIMIENTO_MONEDAS_VISTA.modulo = modulo_mantenimiento_monedas;
	MANTENIMIENTO_MONEDAS_VISTA.rol_jefe = ES_JEFE;
	MANTENIMIENTO_MONEDAS_VISTA.rol_gestor = ES_GESTOR;
	MANTENIMIENTO_MONEDAS_VISTA.inicializar();
}, 500);