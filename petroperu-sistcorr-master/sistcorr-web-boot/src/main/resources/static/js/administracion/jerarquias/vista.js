var modulo_mantenimiento_jerarquia = MODULO_MANTENIMIENTO_JERARQUIA.getInstance(SISTCORR);
var modulo_sistcorr = MODULO_SISTCORR.getInstance(SISTCORR);

var MANTENIMIENTO_JERARQUIAS_VISTA = {
		modSistcorr: null,
		modulo: null,
		errores: [],
		rol_jefe: false,
		...
		obtenerFiltros: function(){
			var ref = this;
			var filtros = JSON.parse(sessionStorage.getItem('FILTROS_ADM_CONSULTA_JERARQUIAS'));
			if(filtros!=null){
				ref.modulo.filtroBusqueda.filtroNombre.val(filtros['nombreJerarquia']);
		...
		iniciarEventos: function(){
			...
			/*ref.modulo.btnEliminarRegistro.off('click').on('click', function(){
				var comp = $(this);
				var idDepartamento = comp.attr("data-jerarquiaid");
			...
			*/
			...
			ref.modulo.compBusqueda.btnLimpiar.click(function(){
				...
				$('#tablaJerarquia').DataTable().clear().destroy();
			...
			ref.modulo.btnRegistrar.click(function(event){
				...
				ref.modulo.compModalDatos.codigoJerarquia.val("");
				ref.modulo.compModalDatos.codigoJerarquia.change();
				ref.modulo.compModalDatos.nombreJerarquia.val("");
				ref.modulo.compModalDatos.nombreJerarquia.change();
				setTimeout(function(){
					ref.modulo.compModalActualizar.modal('show');
				}, 300);
			});
		...
		eventoDocumentoPrincipal: function(){
			...
			ref.modulo.btnGrillaEditar.off('click').on('click', function(){
				...
				var comp = $(this);
				var idJerarquia = comp.attr("data-jerarquiaid");
				
				for(var i in datosConsulta){
					var jerarquia = datosConsulta[i];
					if(jerarquia.id == idJerarquia){
						ref.modulo.compModalDatos.id = jerarquia.id;
						ref.modulo.compModalDatos.accion = "U";
						ref.modulo.compModalDatos.codigoJerarquia.val(jerarquia.codigoJerarquia);
						ref.modulo.compModalDatos.codigoJerarquia.change();
						ref.modulo.compModalDatos.nombreJerarquia.val(jerarquia.nombreJerarquia);
						ref.modulo.compModalDatos.nombreJerarquia.change();
						break;
					}
				}
			...
			ref.modulo.btnGrillaEliminar.off('click').on('click', function(){
				var comp = $(this);
				var idJerarquia = comp.attr("data-jerarquiaid");
				ref.modulo.compModalDatos.id = idJerarquia;
				setTimeout(function(){
					ref.modulo.compModalEliminar.modal('show');
				}, 300);
			}),
			...
			ref.modulo.btnEliminarRegistro.off('click').on('click', function(){
				ref.eliminarJerarquia();
			}),
			
			ref.modulo.btnGrabarRegistro.off('click').on('click', function(){
				ref.insertUpdateJerarquia();
			});
		
			ref.modulo.compModalDatos.codigoJerarquia.on('input', function () {
		...
		buscarJerarquias: function(){
			...
			/*if(!valido){
				...
				var filtrosBusqueda = {
						'nombreJerarquia' : ref.modulo.filtroBusqueda.filtroNombre.val(),
			
				}
				ref.filtrosBusqueda = filtrosBusqueda;
				sessionStorage.setItem("FILTROS_ADM_CONSULTA_JERARQUIAS", JSON.stringify(filtrosBusqueda));
				setTimeout(function(){
					ref.inicializarTablaConsulta();
				}, 1000)
			//}	
		...
		eliminarJerarquia: function(){
			...
			ref.modSistcorr.procesar(true);
				...
				ref.modulo.crudJerarquias(ref.parametros)
		...
		insertUpdateJerarquia: function(){
			var ref = this;
			ref.modSistcorr.procesar(true);
			if(ref.modulo.compModalDatos.codigoJerarquia.val() == "" || ref.modulo.compModalDatos.codigoJerarquia.val() == "0"){
				//valido = true;
			...
			if(ref.modulo.compModalDatos.nombreJerarquia.val() == ""){
			...
			ref.parametros = {
					'id' : 	ref.modulo.compModalDatos.id,
					'codigoJerarquia': ref.modulo.compModalDatos.codigoJerarquia.val(),
					'nombreJerarquia' : ref.modulo.compModalDatos.nombreJerarquia.val(),
			...
			ref.modulo.exportarExcel(ref.filtrosBusqueda)
				.then(function(respuesta){
					if (navigator.appVersion.toString().indexOf('.NET') > 0){
						window.navigator.msSaveOrOpenBlob(respuesta, 'reporte_jerarquias.xlsx');
					} else {
						var a = document.createElement('a');
	                    a.href = URL.createObjectURL(respuesta);
	                    a.download = 'reporte_jerarquias.xlsx';
		...
		inicializarTablaConsulta: function(){
			...
			if(ref.dataTableConsulta){
				...
				ref.dataTableConsulta = ref.modulo.compBusqueda.dataTableConsulta.DataTable({
			        ...
			        "ordering"	: true,
			        ajax: {
			        	"url": ref.modulo.URL_BUSCAR_JERARQUIAS,
			        	"type": "GET",
			        	"data": {
			        		'nombreJerarquia' : ref.modulo.filtroBusqueda.filtroNombre.val(),
			        ...
			        "columns": [
			        	{data: 'id', title: '', defaultContent: '', render: function(data, type, full){
			        		return "<i class='fas fa-edit icon_editar btnGrillaEditar' data-jerarquiaid='"+full.id+"' data-toggle='tooltip' title='Editar'></i>"

			        	}},
			        	{data: 'id', title: '', defaultContent: '', render: function(data, type, full){
			        		return "<i class='fas fa-trash-alt icon_eliminar btnGrillaEliminar' data-jerarquiaid='"+full.id+"' data-toggle='tooltip' title='Eliminar'></i>"
			        	}},
			        	{data: 'id', title: 'Id', defaultContent: ''},
			        	{data: 'codigoJerarquia', title: 'Código Jerarquía', defaultContent: ''},
			        	{data: 'nombreJerarquia', title: 'Nombre Jerarquía', defaultContent: ''}
				...
				ref.modulo.compBusqueda.dataTableConsulta.on( 'order.dt', function () {
					var tableOrder = $("#tablaJerarquia").dataTable();
...
setTimeout(function(){
	MANTENIMIENTO_JERARQUIAS_VISTA.modSistcorr = modulo_sistcorr;
	MANTENIMIENTO_JERARQUIAS_VISTA.modulo = modulo_mantenimiento_jerarquia;
	MANTENIMIENTO_JERARQUIAS_VISTA.rol_jefe = ES_JEFE;
	MANTENIMIENTO_JERARQUIAS_VISTA.rol_gestor = ES_GESTOR;
	MANTENIMIENTO_JERARQUIAS_VISTA.inicializar();
}, 500);