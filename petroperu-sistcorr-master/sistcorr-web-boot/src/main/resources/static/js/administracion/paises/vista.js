var modulo_mantenimiento_pais = MODULO_MANTENIMIENTO_PAIS.getInstance(SISTCORR);
var modulo_sistcorr = MODULO_SISTCORR.getInstance(SISTCORR);

var MANTENIMIENTO_PAIS_VISTA = {
		modSistcorr: null,
		modulo: null,
		errores: [],
		rol_jefe: false,
		...
		obtenerFiltros: function(){
			var ref = this;
			var filtros = JSON.parse(sessionStorage.getItem('FILTROS_ADM_CONSULTA_PAISES'));
			if(filtros!=null){
				ref.modulo.filtroBusqueda.filtroNombre.val(filtros['nombrePais']);
		...
		iniciarEventos: function(){
			...
			/*ref.modulo.btnEliminarRegistro.off('click').on('click', function(){
				var comp = $(this);
				var idDepartamento = comp.attr("data-paisid");
			...
			*/
			...
			ref.modulo.compBusqueda.btnLimpiar.click(function(){
				...
				$('#tablaPaises').DataTable().clear().destroy();
			});
			...
			ref.modulo.btnRegistrar.click(function(event){
				...
				ref.modulo.compModalDatos.codigoPais.val("");
				ref.modulo.compModalDatos.codigoPais.change();
				ref.modulo.compModalDatos.nombrePais.val("");
				ref.modulo.compModalDatos.nombrePais.change();
				setTimeout(function(){
					ref.modulo.compModalActualizar.modal('show');
				}, 300);
			});
		...
		eventoDocumentoPrincipal: function(){
			...
			ref.modulo.btnGrillaEditar.off('click').on('click', function(){
				...
				var comp = $(this);
				var idPais = comp.attr("data-paisid");
				
				for(var i in datosConsulta){
					var pais = datosConsulta[i];
					if(pais.id == idPais){
						ref.modulo.compModalDatos.id = pais.id;
						ref.modulo.compModalDatos.accion = "U";
						ref.modulo.compModalDatos.codigoPais.val(pais.codigoPais);
						ref.modulo.compModalDatos.codigoPais.change();
						ref.modulo.compModalDatos.nombrePais.val(pais.nombrePais);
						ref.modulo.compModalDatos.nombrePais.change();
						break;
					}
				}
			...
			ref.modulo.btnGrillaEliminar.off('click').on('click', function(){
				var comp = $(this);
				var idPais = comp.attr("data-paisid");
				ref.modulo.compModalDatos.id = idPais;
				setTimeout(function(){
					ref.modulo.compModalEliminar.modal('show');
				}, 300);
			}),
			...
			ref.modulo.btnGrabarRegistro.off('click').on('click', function(){
				ref.insertUpdatePais();
			});
		
			ref.modulo.compModalDatos.codigoPais.on('input', function () {
		...
		buscarPaises: function(){
			...
			/*if(!valido){
				...
				var filtrosBusqueda = {
						'nombrePais' : ref.modulo.filtroBusqueda.filtroNombre.val(),
			
				}
				ref.filtrosBusqueda = filtrosBusqueda;
				sessionStorage.setItem("FILTROS_ADM_CONSULTA_PAISES", JSON.stringify(filtrosBusqueda));
				setTimeout(function(){
					ref.inicializarTablaConsulta();
				}, 1000)
			//}	
		...
		eliminarPais: function(){
			...
			ref.modSistcorr.procesar(true);
				...
				ref.modulo.crudPaises(ref.parametros)
		...
		insertUpdatePais: function(){
			...
			if(ref.modulo.compModalDatos.codigoPais.val() == "0"){
				//valido = true;
				ref.modSistcorr.notificar("ERROR", "El campo Código no es válido", "Warning");
				ref.modSistcorr.procesar(false);
				return;
			}

			if(ref.modulo.compModalDatos.nombrePais.val() == ""){
			...
			ref.parametros = {
					...
					'nombrePais' : ref.modulo.compModalDatos.nombrePais.val(),
			...
			ref.modulo.exportarExcel(ref.filtrosBusqueda)
				.then(function(respuesta){
					if (navigator.appVersion.toString().indexOf('.NET') > 0){
						window.navigator.msSaveOrOpenBlob(respuesta, 'reporte_paises.xlsx');
					} else {
						var a = document.createElement('a');
	                    a.href = URL.createObjectURL(respuesta);
	                    a.download = 'reporte_paises.xlsx';
				...
				ref.dataTableConsulta = ref.modulo.compBusqueda.dataTableConsulta.DataTable({
			        ...
			        "columns": [
			        	{data: 'id', title: '', defaultContent: '', render: function(data, type, full){
			        		return "<i class='fas fa-edit icon_editar btnGrillaEditar' data-paisid='"+full.id+"' data-toggle='tooltip' title='Editar'></i>"

			        	}},
			        	{data: 'id', title: '', defaultContent: '', render: function(data, type, full){
			        		return "<i class='fas fa-trash-alt icon_eliminar btnGrillaEliminar' data-paisid='"+full.id+"' data-toggle='tooltip' title='Eliminar'></i>"
			        	}},
			        	{data: 'id', title: 'Id', defaultContent: ''},
			        	{data: 'codigoPais', title: 'Código País', defaultContent: ''},
			        	{data: 'nombrePais', title: 'Nombre País', defaultContent: ''}
...
setTimeout(function(){
	MANTENIMIENTO_PAIS_VISTA.modSistcorr = modulo_sistcorr;
	MANTENIMIENTO_PAIS_VISTA.modulo = modulo_mantenimiento_pais;
	MANTENIMIENTO_PAIS_VISTA.rol_jefe = ES_JEFE;
	MANTENIMIENTO_PAIS_VISTA.rol_gestor = ES_GESTOR;
	MANTENIMIENTO_PAIS_VISTA.inicializar();
}, 500);