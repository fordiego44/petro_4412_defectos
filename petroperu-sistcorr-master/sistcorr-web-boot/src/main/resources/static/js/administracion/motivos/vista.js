var modulo_mantenimiento_motivo = MODULO_MANTENIMIENTO_MOTIVO.getInstance(SISTCORR);
var modulo_sistcorr = MODULO_SISTCORR.getInstance(SISTCORR);

var MANTENIMIENTO_MOTIVO_VISTA = {
		modSistcorr: null,
		modulo: null,
		errores: [],
		rol_jefe: false,
		...
		inicializar: function(){
			...
			var filtros = JSON.parse(sessionStorage.getItem('FILTROS_CONSULTA_MOTIVO'));
			if(filtros!=null){
				ref.modulo.filtroBusqueda.filtroNombre.val(filtros['nombreMotivo']);
		...
		iniciarEventos: function(){
			...
			/*ref.modulo.btnEliminarRegistro.off('click').on('click', function(){
				var comp = $(this);
				var idDepartamento = comp.attr("data-motivoid");
			...
			*/
			...
			ref.modulo.btnRegistrar.click(function(event){
				...
				ref.modulo.compModalDatos.codigoMotivo.val("");
				ref.modulo.compModalDatos.codigoMotivo.change();
				ref.modulo.compModalDatos.nombreMotivo.val("");
				ref.modulo.compModalDatos.nombreMotivo.change();
				setTimeout(function(){
					ref.modulo.compModalActualizar.modal('show');
				}, 300);
			});
		...
		eventoDocumentoPrincipal: function(){
			...
			ref.modulo.btnGrillaEditar.off('click').on('click', function(){
				...
				var comp = $(this);
				var idMotivo = comp.attr("data-motivoid");
				
				for(var i in datosConsulta){
					var motivo = datosConsulta[i];
					if(motivo.id == idMotivo){
						ref.modulo.compModalDatos.id = motivo.id;
						ref.modulo.compModalDatos.accion = "U";
						ref.modulo.compModalDatos.codigoMotivo.val(motivo.codigoMotivo);
						ref.modulo.compModalDatos.codigoMotivo.change();
						ref.modulo.compModalDatos.nombreMotivo.val(motivo.nombreMotivo);
						ref.modulo.compModalDatos.nombreMotivo.change();
						break;
					}
				}
			...
			ref.modulo.btnGrillaEliminar.off('click').on('click', function(){
				var comp = $(this);
				var idMotivo = comp.attr("data-motivoid");
				ref.modulo.compModalDatos.id = idMotivo;
				setTimeout(function(){
					ref.modulo.compModalEliminar.modal('show');
				}, 300);
			}),
		...
		buscarMotivos: function(){
			...
			/*if(!valido){
				...
				var filtrosBusqueda = {
						'nombreMotivo' : ref.modulo.filtroBusqueda.filtroNombre.val(),
			
				}
				ref.filtrosBusqueda = filtrosBusqueda;
				sessionStorage.setItem("FILTROS_CONSULTA_MOTIVO", JSON.stringify(filtrosBusqueda));
				ref.inicializarTablaConsulta();
			//}	
		},
		
		
		eliminarMotivos: function(){
			var ref = this;
			ref.parametros = {
			...
			ref.modSistcorr.procesar(true);
				...
				ref.modulo.crudMotivos(ref.parametros)
		...
		insertUpdateMotivo: function(){
			var ref = this;
			ref.modSistcorr.procesar(true);
			if(ref.modulo.compModalDatos.codigoMotivo.val().trim() == ""){
				//valido = true;
			...
			if(ref.modulo.compModalDatos.nombreMotivo.val().trim() == ""){
			...
			ref.parametros = {
					'id' : 	ref.modulo.compModalDatos.id,
					'codigoMotivo': ref.modulo.compModalDatos.codigoMotivo.val(),
					'nombreMotivo' : ref.modulo.compModalDatos.nombreMotivo.val(),
			...
			ref.modulo.exportarExcel(ref.filtrosBusqueda)
				.then(function(respuesta){
					if (navigator.appVersion.toString().indexOf('.NET') > 0){
						window.navigator.msSaveOrOpenBlob(respuesta, 'reporte_motivos.xlsx');
					} else {
						var a = document.createElement('a');
	                    a.href = URL.createObjectURL(respuesta);
	                    a.download = 'reporte_motivos.xlsx';
		...
		inicializarTablaConsulta: function(){
			...
			if(ref.dataTableConsulta){
				...
				ref.dataTableConsulta = ref.modulo.compBusqueda.dataTableConsulta.DataTable({
			        ...
			        "ordering"	: true,
			        ajax: {
			        	"url": ref.modulo.URL_BUSCAR_MOTIVOS,
			        	"type": "GET",
			        	"data": {
			        		'nombreMotivo' : ref.modulo.filtroBusqueda.filtroNombre.val(),
			        	...
			        	'error': function(result){
			        		console.log("Error Consulta Motivo");
			        		console.log(result);
			        		ref.modSistcorr.procesar(false);
			        		//location.reload();
			        	}
			        ...
			        "columns": [
			        	{data: 'id', title: '', defaultContent: '', render: function(data, type, full){
			        		return "<i class='fas fa-edit icon_editar btnGrillaEditar' data-motivoid='"+full.id+"' data-toggle='tooltip' title='Editar'></i>"

			        	}},
			        	{data: 'id', title: '', defaultContent: '', render: function(data, type, full){
			        		return "<i class='fas fa-trash-alt icon_eliminar btnGrillaEliminar' data-motivoid='"+full.id+"' data-toggle='tooltip' title='Eliminar'></i>"
			        	}},
			        	{data: 'id', title: 'Id', defaultContent: ''},
			        	{data: 'codigoMotivo', title: 'Código Motivo', defaultContent: ''},
			        	{data: 'nombreMotivo', title: 'Nombre Motivo', defaultContent: ''}
				...
				ref.modulo.compBusqueda.dataTableConsulta.on( 'order.dt', function () {
					var tableOrder = $("#tablaMotivos").dataTable();
...
setTimeout(function(){
	MANTENIMIENTO_MOTIVO_VISTA.modSistcorr = modulo_sistcorr;
	MANTENIMIENTO_MOTIVO_VISTA.modulo = modulo_mantenimiento_motivo;
	MANTENIMIENTO_MOTIVO_VISTA.rol_jefe = ES_JEFE;
	MANTENIMIENTO_MOTIVO_VISTA.rol_gestor = ES_GESTOR;
	MANTENIMIENTO_MOTIVO_VISTA.inicializar();
}, 500);