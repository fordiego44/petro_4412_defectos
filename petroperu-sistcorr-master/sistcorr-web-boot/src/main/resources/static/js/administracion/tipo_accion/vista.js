...
var MANTENIMIENTO_LUGAR_TRABAJO_VISTA = {
		...
		obtenerFiltros: function(){
			var ref = this;
			var filtros = JSON.parse(sessionStorage.getItem('FILTROS_ADM_TIPO_ACCION'));
			if(filtros!=null){
		
				ref.modulo.filtroBusqueda.filtroNombre.val(filtros['nombreAccion']);
		...
		/*	ref.modulo.lblFiltroEstado.click(function(){
			...
			*/	
			...
			ref.modulo.btnRegistrar.click(function(event){
				...
				ref.modulo.compModalDatos.codigoAccion.val("");
				ref.modulo.compModalDatos.codigoAccion.change();
				ref.modulo.compModalDatos.nombreAccion.val("");
				ref.modulo.compModalDatos.nombreAccion.change();
				
				ref.modulo.compModalDatos.mTextoReq.val("");
				ref.modulo.compModalDatos.mTextoReq.change();
				ref.modulo.compModalDatos.mMultipli.val("");
				ref.modulo.compModalDatos.mMultipli.change();
				ref.modulo.compModalDatos.mReqTextoRta.val("");
				ref.modulo.compModalDatos.mReqTextoRta.change();
				ref.modulo.compModalDatos.mEnviaMailRta.val("");
				ref.modulo.compModalDatos.mEnviaMailRta.change();
		...
		eventoDocumentoPrincipal: function(){
			...
			ref.modulo.btnGrillaEditar.off('click').on('click', function(){
				...
				var comp = $(this);
				var idTipoAccion = comp.attr("data-tipoaccionid");
				
				for(var i in datosConsulta){
					var datoTipoAccion = datosConsulta[i];
					if(datoTipoAccion.id == idTipoAccion){
						ref.modulo.compModalDatos.id = datoTipoAccion.id;
						ref.modulo.compModalDatos.codigoAccion.val(datoTipoAccion.codigoAccion);
						ref.modulo.compModalDatos.codigoAccion.change();
						ref.modulo.compModalDatos.nombreAccion.val(datoTipoAccion.nombreAccion);
						ref.modulo.compModalDatos.nombreAccion.change();
						ref.modulo.compModalDatos.mTextoReq.val(datoTipoAccion.mTextoReq);
						ref.modulo.compModalDatos.mTextoReq.change();
						ref.modulo.compModalDatos.mMultipli.val(datoTipoAccion.mMultipli);
						ref.modulo.compModalDatos.mMultipli.change();
						ref.modulo.compModalDatos.mReqTextoRta.val(datoTipoAccion.mReqTextoRta);
						ref.modulo.compModalDatos.mReqTextoRta.change();
						ref.modulo.compModalDatos.mEnviaMailRta.val(datoTipoAccion.mEnviaMailRta);
						ref.modulo.compModalDatos.mEnviaMailRta.change();
			...
			ref.modulo.btnGrillaEliminar.off('click').on('click', function(){
				var comp = $(this);
				var idTipoAccion = comp.attr("data-tipoaccionid");
				ref.modulo.compModalDatos.id = idTipoAccion;
				setTimeout(function(){
					ref.modulo.compModalEliminar.modal('show');
				}, 300);
			}),
		...
		buscarTipoAccion: function(){
			...
			/*if(!valido){
				...
				var filtrosBusqueda = {
						'nombreAccion' : ref.modulo.filtroBusqueda.filtroNombre.val()
			
				}
				ref.filtrosBusqueda = filtrosBusqueda;
				sessionStorage.setItem("FILTROS_ADM_TIPO_ACCION", JSON.stringify(filtrosBusqueda));
				setTimeout(function(){
					ref.inicializarTablaConsulta();
				}, 1000)
		...
		insertUpdateTipoAccion: function(){
			var ref = this;
			ref.modSistcorr.procesar(true);
			if(ref.modulo.compModalDatos.codigoAccion.val().trim() == ""){
				ref.modSistcorr.notificar("ERROR", "El campo Código es obligatorio", "Warning");
				ref.modSistcorr.procesar(false);
				return;
			}
			if(ref.modulo.compModalDatos.nombreAccion.val().trim() == ""){
				ref.modSistcorr.notificar("ERROR", "El campo Acción es obligatorio", "Warning");
				ref.modSistcorr.procesar(false);
				return;
			}
			
			if(ref.modulo.compModalDatos.mTextoReq.val().trim() == ""){
				//valido = true;
				ref.modSistcorr.notificar("ERROR", "El campo Text Req es obligatorio", "Warning");
				ref.modSistcorr.procesar(false);
				return;
			}

			if(ref.modulo.compModalDatos.mMultipli.val().trim() == ""){
				ref.modSistcorr.notificar("ERROR", "El campo Múltiple es obligatorio", "Warning");
				ref.modSistcorr.procesar(false);
				return;
			}
			
			if(ref.modulo.compModalDatos.mReqTextoRta.val().trim() == ""){
				ref.modSistcorr.notificar("ERROR", "El campo MarcaReq. Text Resp. es obligatorio", "Warning");
				ref.modSistcorr.procesar(false);
				return;
			}
			if(ref.modulo.compModalDatos.mEnviaMailRta.val().trim() == ""){
				ref.modSistcorr.notificar("ERROR", "El campo Marca Env. Mail Rta es obligatorio", "Warning");
				ref.modSistcorr.procesar(false);
				return;
			}
			...
			ref.parametros = {
					'id' : 	ref.modulo.compModalDatos.id,
					'codigoAccion'	 : ref.modulo.compModalDatos.codigoAccion.val(),
					'nombreAccion'   : ref.modulo.compModalDatos.nombreAccion.val(),					
					'mTextoReq'   	 : ref.modulo.compModalDatos.mTextoReq.val(),
					'mMultipli'   	 : ref.modulo.compModalDatos.mMultipli.val(),
					'mReqTextoRta'   : ref.modulo.compModalDatos.mReqTextoRta.val(),
					'mEnviaMailRta'  : ref.modulo.compModalDatos.mEnviaMailRta.val(),
					'accion'         : ref.modulo.compModalDatos.accion
			};
				ref.modulo.crudLugarTrabajo(ref.parametros)
					.then(function(respuesta){
						console.log("Respuesta.estado:" + respuesta.estado)
			...
			ref.modulo.exportarExcel(filtrosBusqueda)
				.then(function(respuesta){
					if (navigator.appVersion.toString().indexOf('.NET') > 0){
						window.navigator.msSaveOrOpenBlob(respuesta, 'reporte_tipo_accion.xlsx');
					} else {
						var a = document.createElement('a');
	                    a.href = URL.createObjectURL(respuesta);
	                    a.download = 'reporte_tipo_accion.xlsx';
				...
				ref.dataTableConsulta = ref.modulo.compBusqueda.dataTableConsulta.DataTable({
			        ...
			        "columns": [
			        	{data: 'id', title: '', defaultContent: '', render: function(data, type, full){
			        		return "<i class='fas fa-edit icon_editar btnGrillaEditar' data-tipoaccionid='"+full.id+"' data-toggle='tooltip' title='Editar'></i>"

			        	}},
			        	{data: 'id', title: '', defaultContent: '', render: function(data, type, full){
			        		return "<i class='fas fa-trash-alt icon_eliminar btnGrillaEliminar' data-tipoaccionid='"+full.id+"' data-toggle='tooltip' title='Eliminar'></i>"
			        	}},
			        	{data: 'id', title: 'Id', defaultContent: ''},
			        	{data: 'codigoAccion', title: 'Código', defaultContent: ''},
			        	{data: 'nombreAccion', title: 'Nombre Acción', defaultContent: ''},
			        	{data: 'mTextoReq', title: 'Text Req', defaultContent: ''},
			        	{data: 'mMultipli', title: 'Múltiple', defaultContent: ''},
			        	{data: 'mReqTextoRta', title: 'Marca Req.Text Resp.', defaultContent: ''},
			        	{data: 'mEnviaMailRta', title: 'Marca Env.Mail.Rta', defaultContent: ''}			        	
